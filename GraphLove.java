...
//        System.out.println();
        var result = getPath(8, 2, solve(0));
        Collections.reverse(result);
        System.out.println(result);
    }

    public static List<Integer> solve(Integer s) {
        List<Integer> prev = Arrays.asList(new Integer[n+1]);
        List<Boolean> visted = IntStream.range(0, n+1).boxed().map(x -> Boolean.FALSE).collect(Collectors.toList());
        Queue<Integer> qq = new LinkedList<>();
        qq.offer(s);
        while (!qq.isEmpty()) {
            var item = qq.poll();
            visted.set(item, Boolean.TRUE);
            var neibors = graph.get(item);
            neibors.forEach(neibor -> {
                if (!visted.get(neibor)) {
                    qq.offer(neibor);
                    prev.set(neibor, item);
                }
            });
        }
        return prev;
...
//
//    }
    public static List<Integer> getPath(Integer s, Integer e,List<Integer> map) {
